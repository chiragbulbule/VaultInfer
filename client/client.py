...
VAULT_MED       = BASE_DIR.parent / "model" / "vault_med_model"
FEATURE_DATA    = VAULT_MED / "feature_extraction" / "data"
...
weights_dict = load(FEATURE_DATA / "weights.pth.tar", map_location="cpu")
...
scaler  = jl.load(FEATURE_DATA / "vault_med_r_scaler.joblib")
clipper = jl.load(FEATURE_DATA / "vault_med_clipper.joblib")
...
def predict(image_path: str, server_url: str = "http://localhost:8000"):
    ...
    print(f"\nScore (logit)            : {score:.4f}")
    print(f"Probability of Pneumonia : {prob * 100:.2f}%")
