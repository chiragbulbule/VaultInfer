...
model=jl.load(f"{PARENT_DIR}/model_training/vault_med_model")

#--------------------------------------------------------------------------------------TESTING-Range of Weights ,Weighted Sum and Features--------------------------------------------------------------------------------------#

"""
weights=np.load(f"{PARENT_DIR}/model_training/vault_weights.npy")
bias=np.load(f"{PARENT_DIR}/model_training/vault_bias.npy")
features=np.load(f"{PARENT_DIR}/feature_extraction/data/train_features.npy")
...
weights_dict=load(f"{PARENT_DIR}/feature_extraction/data/weights.pth.tar")
scaler=jl.load(f"{PARENT_DIR}/feature_extraction/data/vault_med_r_scaler.joblib")
clipper=jl.load(f"{PARENT_DIR}/feature_extraction/data/vault_med_clipper.joblib")
...
image_1=Image.open(f"{BASE_DIR}/inconclusive.jpg").convert("RGB")
...
test_features=np.load(f"{PARENT_DIR}/feature_extraction/data/test_features.npy")
test_labels=np.load(f"{PARENT_DIR}/feature_extraction/data/test_labels.npy")
...
PARENT_DIR}/feature_extraction/data/test_labels.npy")

prob = model.predict_proba(test_features)[:,1]

pl.hist(prob[test_labels == 0],bins=30,alpha=0.5,label="NORMAL")
pl.hist(prob[test_labels == 1],bins=30,alpha=0.5,label="PNEUMONIA")
pl.legend()
pl.show()
